...
def check_out():
        ...
        clear_screen()
        show_title ("CHECKOUT")
        print(tabulate(df_kamar,headers="firstrow", tablefmt="fancy_grid"))
        nomor_kamar = int(input("\nMasukkan Nomor Kamar yang mau di checkout: "))
        kamar = df_kamar[df_kamar["Nomor Kamar"] == nomor_kamar]
    
        if not kamar.empty and kamar.iloc[0]["Status"] == "Terisi":
            df_kamar.loc[df_kamar["Nomor Kamar"] == nomor_kamar, "Status"] = "Kosong"
            df_kamar.to_csv("data_kamar.csv", index=False)
            print(f"Kamar {nomor_kamar} kini telah kosong.")
        else:
            print("Kamar tidak ditemukan atau sudah kosong.")
...
def atur_rawat_inap():
    ...
    if not kamar.empty and kamar.iloc[0]["Status"] == "Kosong":
        df_kamar.loc[df_kamar["Nomor Kamar"] == nomor_kamar, "Status"] = "Terisi"
        df_kamar.to_csv("data_kamar.csv", index=False)
